...
export function ProjectsList({ initialProjects }: { initialProjects: any[] }) {
  ...
  const filteredProjects = projects.filter((project) => {
    const matchesSearch =
      project.pid.toLowerCase().includes(search.toLowerCase()) ||
    ...
    <div className="space-y-6">
      <div className="flex gap-4">
        <div className="flex-1 relative">
          ...
          <Input
            placeholder="Search project by PID, customer, sales, or distributor..."
            value={search}
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-slate-200 dark:border-slate-700">
                    ...
                    <th className="text-left py-3 px-4 text-slate-700 dark:text-slate-300">Sales</th>
                    <th className="text-left py-3 px-4 text-slate-700 dark:text-slate-300">Products</th>
                    <th className="text-left py-3 px-4 text-slate-700 dark:text-slate-300">Presales</th>
                    <th className="text-left py-3 px-4 text-slate-700 dark:text-slate-300">Engineers</th>
                    <th className="text-left py-3 px-4 text-slate-700 dark:text-slate-300">Distributor</th>
                ...
                <tbody>
                  {filteredProjects.map((project) => (
                    ...
                    >
                        ...
                        {project.project_engineers && project.project_engineers.length > 0 ? (
                          <div className="flex flex-wrap gap-1">
                            {project.project_engineers.map((pe: any) => (
                              <Badge
                                key={pe.user?.id}
                                className="bg-orange-900 text-orange-200 text-xs"
                              >
                      ...
                      <td className="py-3 px-4 text-slate-600 dark:text-slate-400">{project.distributor?.name || "-"}</td>
                      <td className="py-3 px-4 text-slate-600 dark:text-slate-400">
