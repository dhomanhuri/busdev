...
import { ProductsList } from "@/components/master/products-list";
...
export default async function ProductsPage() {
  ...
  const activeProducts = products?.filter(p => p.status_aktif).length || 0;
  const uniqueCategories = new Set(products?.map(p => p.brand?.sub_category?.category?.name).filter(Boolean) || []).size;
  const uniqueBrands = new Set(products?.map(p => p.brand?.name).filter(Boolean) || []).size;
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <div className="group relative bg-white/80 dark:bg-slate-900/80 backdrop-blur-sm border border-slate-200/60 dark:border-slate-800/60 rounded-2xl p-6 hover:border-blue-300 dark:hover:border-blue-700/50 hover:shadow-xl hover:shadow-blue-500/10 dark:hover:shadow-blue-900/20 transition-all duration-300 hover:-translate-y-1">
          ...
          <div className="relative flex items-center justify-between mb-4">
            <p className="text-sm font-semibold text-slate-600 dark:text-slate-400">Total Products</p>
          ...
          <p className="relative text-xs text-slate-500 dark:text-slate-400 font-medium">{activeProducts} active</p>
        </div>
        ...
        <div className="group relative bg-white/80 dark:bg-slate-900/80 backdrop-blur-sm border border-slate-200/60 dark:border-slate-800/60 rounded-2xl p-6 hover:border-emerald-300 dark:hover:border-emerald-700/50 hover:shadow-xl hover:shadow-emerald-500/10 dark:hover:shadow-emerald-900/20 transition-all duration-300 hover:-translate-y-1">
          ...
          <div className="relative flex items-center justify-between mb-4">
            <p className="text-sm font-semibold text-slate-600 dark:text-slate-400">Active Products</p>
          ...
          <p className="relative text-4xl font-extrabold text-slate-900 dark:text-slate-50 mb-2 tracking-tight">{activeProducts}</p>
        ...
        <div className="group relative bg-white/80 dark:bg-slate-900/80 backdrop-blur-sm border border-slate-200/60 dark:border-slate-800/60 rounded-2xl p-6 hover:border-purple-300 dark:hover:border-purple-700/50 hover:shadow-xl hover:shadow-purple-500/10 dark:hover:shadow-purple-900/20 transition-all duration-300 hover:-translate-y-1">
          ...
          <p className="relative text-xs text-slate-500 dark:text-slate-400 font-medium">Unique categories</p>
        </div>
        ...
        <div className="group relative bg-white/80 dark:bg-slate-900/80 backdrop-blur-sm border border-slate-200/60 dark:border-slate-800/60 rounded-2xl p-6 hover:border-orange-300 dark:hover:border-orange-700/50 hover:shadow-xl hover:shadow-orange-500/10 dark:hover:shadow-orange-900/20 transition-all duration-300 hover:-translate-y-1">
          ...
          <div className="relative flex items-center justify-between mb-4">
            <p className="text-sm font-semibold text-slate-600 dark:text-slate-400">Brands</p>
          ...
          <p className="relative text-4xl font-extrabold text-slate-900 dark:text-slate-50 mb-2 tracking-tight">{uniqueBrands}</p>
          <p className="relative text-xs text-slate-500 dark:text-slate-400 font-medium">Unique brands</p>
        </div>
      </div>
